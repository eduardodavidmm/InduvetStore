...
const UserEditScreen = ({ match, history }) => {
    ...
    return (
        ...
        <>
            <Link to='/admin/userlist' className='btn btn-success my-3 mtopone'>
                Regresar
            </Link>

            <FormContainer>
            <h1 className="lel">Editar Usuario</h1>
            ...
            {loading ? <Loader/> : error ? <Message variant='danger'>{error}</Message> : (
               ...
               <Form.Group controlId='isadmin'>
                   <Form.Check type='checkbox' label='Administrador' value={isAdmin} checked={isAdmin} onChange={(e) => setIsAdmin(e.target.checked)}></Form.Check>
               </Form.Group>
